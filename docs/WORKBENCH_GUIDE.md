...
Public hosting uses the Dockerfile, `backend.serve` launcher and exact host/origin
configuration described in [DEPLOYMENT.md](../DEPLOYMENT.md). `CONTOUR_PUBLIC=1`
disables saved/environment key fallback, enables bounded global request admission,
and accepts only configured public hostnames. The service has no user accounts or
per-user cloud storage; shared limits are not tenant isolation. See the deployment
guide before exposing the service. Local development still uses `python run.py`.
